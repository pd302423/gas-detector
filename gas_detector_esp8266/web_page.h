...
.cmd {
  font-family: ui-monospace, "SF Mono", SFMono-Regular, Menlo, Consolas, monospace;
  font-size: .84rem; line-height: 1.6; background: var(--bg-sunken);
  border: 1px solid var(--line); border-radius: 10px;
  padding: 12px 14px; margin: 10px 0 0; overflow-x: auto; white-space: pre;
  color: var(--text-2);
}
.cmd b { color: var(--text); font-weight: 600; }

/* Numbered steps — the count is meaningful here, the order is the instruction */
...
.steps p { margin: 0; color: var(--text-2); font-size: .92rem; }
ol.steps, ul.steps { padding: 0; margin: 0; }
...
<header class="nav">
  <div class="nav-in">
    ...
    <div class="seg" role="tablist" id="tabs">
      ...
      <button role="tab" aria-selected="false" data-p="setup">Setup</button>
...
<main>
  ...
  <section class="panel stack" id="p-live">
    <div class="card wide" id="connectBanner" hidden
         style="display:flex;gap:18px;align-items:center;flex-wrap:wrap;
                border-color:var(--accent);background:var(--fill)">
      <div style="flex:1 1 300px;min-width:240px">
        <div class="eyebrow" id="connectEyebrow">Not connected</div>
        <div id="connectTitle" style="font-size:1.15rem;font-weight:650;letter-spacing:-.012em;
             margin-top:3px">You are looking at simulated data</div>
        <p class="sub" id="connectWhy" style="margin:7px 0 0;max-width:62ch"></p>
      </div>
      <button class="btn primary" id="btnUsbBig" style="font-size:1rem;padding:12px 22px"
              hidden>Connect detector</button>
    </div>
    ...
    <div class="grid">
      ...
      <div class="card">
        <div class="card-h"><div><h2>Alerts</h2>
          <div class="sub" id="alertSub">Notify someone when the alarm latches</div></div>
          <span class="tag" id="alertTag">off</span></div>
        <label class="sub" for="alertTo" style="display:block;margin-bottom:6px">
          Email address, or phone with country code</label>
        <input id="alertTo" type="text" inputmode="email" autocomplete="off" spellcheck="false"
          placeholder="you@example.com  or  +919876543210"
          style="width:100%;font:inherit;font-size:.92rem;padding:9px 12px;border-radius:10px;
                 border:1px solid var(--line);background:var(--bg);color:var(--text)">
        <div class="row mt">
          <button class="btn primary" id="btnArm">Arm alerts</button>
          <button class="btn" id="btnAlertTest">Send test</button>
        </div>
        <p class="sub mt" id="alertMsg" style="min-height:2.4em"></p>
      </div>
  ...
  <!-- ================= SETUP ================= -->
  <section class="panel stack" id="p-setup" hidden>
    <div class="card wide">
      <div class="card-h"><div class="eyebrow">Running this yourself</div></div>
      <h1>Four steps to live readings.</h1>
      <p class="sub mt" style="max-width:62ch">If the detector is already flashed and calibrated,
        this is the whole procedure. First-time setup is below it.</p>

      <ol class="steps mt" style="margin-top:22px">
        <li><h3>Plug the Uno into this computer</h3>
          <p>One USB cable powers the board and carries the data. It must be a <b>data</b> cable —
            many charging cables have no data lines and the port simply never appears.</p></li>
        <li><h3>Close the Arduino IDE's Serial Monitor</h3>
          <p>Only one program can hold a serial port at a time. The Serial Monitor still holding it
            is the single most common reason the next step fails.</p></li>
        <li><h3>Open this page in Chrome or Edge, on a computer</h3>
          <p>Reading a USB port needs the Web Serial API, which Firefox and Safari do not have and
            no mobile browser has. The page must also be on <b>https</b> or <b>localhost</b> — a
            file opened from disk cannot touch a port.</p></li>
        <li><h3>Click <b>Connect detector</b> and choose the port</h3>
          <p>The pill in the header turns to <b>USB · live</b> and every tab fills with real data.
            If the button is not visible, the Live tab says which of the conditions above is
            missing.</p></li>
      </ol>

      <p class="note mt">Connecting resets the board — that is unavoidable with USB serial. The
        firmware detects that power never actually dropped and settles in <b>8 seconds</b> instead
        of restarting the full 180-second heater warm-up.</p>
    </div>

    <div class="card wide">
      <div class="card-h"><div><h2>First-time setup</h2>
        <div class="sub">From a box of parts to a working detector</div></div></div>

      <ol class="steps mt">
        <li><h3>Wire it — ten wires, no resistors</h3>
          <p>MQ-135 <code>VCC→5V</code> <code>GND→GND</code> <code>AOUT→A0</code>
            <code>DOUT→D7</code>; LCD backpack <code>VCC→5V</code> <code>GND→GND</code>
            <code>SDA→A4</code> <code>SCL→A5</code>; buzzer long leg <code>→D8</code>, short leg
            <code>→GND</code>. The alarm LED is the Uno's own pin-13 one — nothing to wire.
            The <b>Wiring</b> tab shows every connection; tap a wire to see what it carries.</p></li>

        <li><h3>Check the sensor actually works</h3>
          <p>Upload <code>sensor_check/sensor_check.ino</code> first. It needs only three wires and
            ignores the LCD, so it still reports the truth if the display is dead. Three things
            prove the sensor is alive: <b>it gets warm</b> within a minute, the reading is
            <b>plausible</b> rather than zero or pinned, and it <b>moves</b> when you wave hand
            sanitiser at it. The last one is the one that matters — a dead sensing layer still
            reads a convincing steady number.</p></li>

        <li><h3>Install the one library and flash the firmware</h3>
          <p>Arduino IDE → <b>Tools → Manage Libraries</b> → install <b>LiquidCrystal I2C</b> by
            Frank de Brabander. Open <code>gas_detector_uno/gas_detector_uno.ino</code>, confirm
            <code>MQ_SELECT</code> matches the number on your sensor's can, set
            <code>BUZZER_PASSIVE</code> (steady tone from DC = <code>0</code>, single click =
            <code>1</code>), and upload.</p></li>

        <li><h3>Burn it in for 24–48 hours</h3>
          <p>A new MQ sensor's baseline drifts for that long. Leave it powered overnight, twice.
            Readings taken before it settles do not mean anything, and no amount of calibration
            fixes that.</p></li>

        <li><h3>Calibrate R₀ in clean air</h3>
          <p>Take it <b>outdoors</b> — not the kitchen, not a room where anyone has cooked or used
            sanitiser. Serial Monitor at <b>115200</b>, send <code>c</code>. That measures the
            sensor's clean-air resistance and stores it in EEPROM, where it survives being
            unplugged. Every number this dashboard shows is a ratio against it, so calibrating in a
            contaminated room makes everything downstream quietly wrong. Send <code>r</code> any
            time for raw diagnostics.</p></li>
      </ol>
    </div>

    <div class="grid">
      <div class="card">
        <div class="card-h"><div><h2>Commands</h2>
          <div class="sub">Everything you need on the computer side</div></div></div>

        <p class="sub" style="margin:0">Serve the dashboard locally</p>
        <pre class="cmd">cd dashboard
npx serve</pre>

        <p class="sub mt" style="margin-top:16px">Deploy it to the web</p>
        <pre class="cmd">npm i -g vercel
cd dashboard
vercel --prod</pre>

        <p class="sub mt" style="margin-top:16px">Rebuild after editing the dashboard</p>
        <pre class="cmd">powershell -File tools/build_web_page.ps1</pre>

        <p class="note mt">Deploy from <b>inside</b> <code>dashboard/</code> so
          <code>index.html</code> lands at the root. If you connect the GitHub repo to Vercel
          instead, set <b>Root Directory</b> to <code>dashboard</code>.</p>
      </div>

      <div class="card">
        <div class="card-h"><div><h2>Serial commands</h2>
          <div class="sub">Serial Monitor at 115200</div></div></div>
        <div class="tw"><table style="min-width:280px">
          <thead><tr><th class="n">Key</th><th>Does</th></tr></thead>
          <tbody>
            <tr><td class="n"><code>c</code></td><td>Recalibrate R₀. Outdoors only.</td></tr>
            <tr><td class="n"><code>r</code></td><td>Raw diagnostics — volts, Rs, R₀, ratio, and
              the clamped and unclamped ppm with its range flag for every gas.</td></tr>
          </tbody></table></div>
        <p class="note warn mt">Close the Serial Monitor before connecting the dashboard. One
          program per port.</p>
      </div>
    </div>

    <div class="card wide">
      <div class="card-h"><div><h2>When it doesn't work</h2></div></div>
      <div class="tw"><table>
        <thead><tr><th>Symptom</th><th>Cause</th></tr></thead>
        <tbody>
          <tr><td>LCD blank, or two rows of solid blocks</td><td class="sub">Contrast. Turn the blue trimmer on the back of the I²C backpack before suspecting anything else.</td></tr>
          <tr><td><code>No I2C device found</code></td><td class="sub">SDA/SCL swapped, backpack unpowered, or a cold solder joint on the 16-pin header.</td></tr>
          <tr><td>No port in Arduino IDE</td><td class="sub">Charge-only USB cable, or a missing CH340 driver on an Uno clone.</td></tr>
          <tr><td>No <b>Connect detector</b> button</td><td class="sub">Page is on <code>file://</code> or plain http, is embedded in another page, or the browser is not Chrome/Edge. The Live tab names which.</td></tr>
          <tr><td>Connect fails, port list empty</td><td class="sub">Same cable and driver causes as above — if the Arduino IDE cannot see it, neither can the browser.</td></tr>
          <tr><td>Connect fails with an error</td><td class="sub">Something else holds the port. Close the Serial Monitor.</td></tr>
          <tr><td>ppm pinned at maximum</td><td class="sub">R₀ was calibrated in dirty air. Redo it outdoors.</td></tr>
          <tr><td>Readings drift for hours</td><td class="sub">Normal on a new sensor. Finish the burn-in.</td></tr>
          <tr><td>Readings shift with the weather</td><td class="sub">Real — SnO₂ is humidity-sensitive. Compensation is on the Roadmap tab.</td></tr>
          <tr><td>Alerts card says unavailable</td><td class="sub">No API key on this deployment, or no server behind this copy of the page. The card says which.</td></tr>
        </tbody></table></div>
    </div>

    <div class="card wide">
      <div class="card-h"><div><h2>Source and full documentation</h2></div></div>
      <div class="prose">
        <p>Everything — firmware, this dashboard, the wiring reference, the booth guide — is at
          <a href="https://github.com/pd302423/gas-detector" target="_blank" rel="noopener"
             style="color:var(--accent)">github.com/pd302423/gas-detector</a>.</p>
        <p><code>docs/SETUP.md</code> in that repository is the long-form version of this page,
          including deploying your own copy and switching on email or SMS leak alerts.
          <code>docs/WIRING.md</code> is the printable wiring sheet.</p>
        <p class="note danger">This is a demonstration and teaching instrument. It has no
          self-test, no sensor-failure detection, no battery backup and no certification, and its
          calibration traces to a curve fit against a datasheet graph rather than a reference gas.
          For actual protection of a home with an LPG cylinder, buy a BIS or CE certified alarm.</p>
      </div>
    </div>
  </section>
...
let lastUsbLine = 0, warmupLeft = 0, connectError = null;
...
async function connectUsb() {
  ...
  try {
    ...
    // Surfaced in the connect banner rather than tucked under the hero — this
    // is connection state, and the banner is where the user is already looking.
    connectError = (e && e.name === "NotFoundError")
      ? {kind: "no_port"}
      : {kind: "open_failed", detail: (e && e.message) || String(e)};
    port = null;
    paintConnectBanner();
  }
}
...
/* ===========================================================================
   ...
   The page never holds a key. It decides *that* an alert is warranted and
   posts to /api/alert, which runs on the server and holds the credentials.
   Served from anywhere without those environment variables — a fresh clone,
   a local file — the endpoint reports itself unconfigured and this panel
   says so plainly rather than failing silently.
   =========================================================================== */
let alertCfg = null;          // null until /api/alert answers
let alertArmed = false;
let alertLastStatus = "SAFE"; // edge detection: fire on entry, not every poll

function alertNote(text, tone) {
  const el = $("#alertMsg");
  el.innerHTML = text;
  el.style.color = tone ? `var(--${tone})` : "var(--text-3)";
}

function paintAlertUI() {
  const tag = $("#alertTag"), arm = $("#btnArm");
  const usable = alertCfg && alertCfg.configured;

  $("#alertTo").disabled = !usable;
  $("#btnAlertTest").disabled = !usable;
  arm.disabled = !usable;
  ...
  const ch = [alertCfg.channels.email && "email", alertCfg.channels.sms && "SMS"]
    .filter(Boolean).join(" and ");
  $("#alertSub").textContent = `Notify someone by ${ch} when the alarm latches`;
  tag.textContent = alertArmed ? "armed" : "off";
  tag.className = "tag " + (alertArmed ? "SAFE" : "");
  arm.textContent = alertArmed ? "Disarm" : "Arm alerts";
}
...
async function sendAlert({test = false} = {}) {
  const to = $("#alertTo").value.trim();
  if (!to) { alertNote("Enter an email address or phone number first.", "warn"); return; }

  const w = worstGas();
  const lel = {LPG: 18000, CH4: 50000, H2: 40000}[w.name];
  const payload = {
    to, test,
    gas: w.name,
    ppm: ppmFmt(w.ppm),
    unit: MQ.unit,
    status: test ? "WARNING" : live.status,
    ratio: fmt(live.ratio, 3),
    pctLEL: lel ? fmt(100 * w.ppm / lel, 1) : null,
  };
  ...
  try {
    ...
    if (r.ok) {
      alertNote(`Sent by ${out.via} to <b>${to}</b> at ${new Date().toLocaleTimeString()}.`, "safe");
    } else {
      alertNote(out.message || `Failed (HTTP ${r.status}).`, "danger");
    }
  } catch (e) {
    alertNote("Could not reach the alert endpoint: " + e.message, "danger");
  }
}

/* Called from render(). Fires once on the transition into an alarm state —
   not on every poll, which at two-second intervals would be a message storm.
   The server enforces its own cooldown as a second line of defence. */
function maybeAlert() {
  if (!live) return;
  const now = live.status;
  const entering = now !== "SAFE" && alertLastStatus === "SAFE";
  alertLastStatus = now;
  if (!entering || !alertArmed || !alertCfg || !alertCfg.configured) return;
  sendAlert();
}

$("#btnArm").addEventListener("click", () => {
  const to = $("#alertTo").value.trim();
  if (!alertArmed && !to) { alertNote("Enter a recipient before arming.", "warn"); return; }
  alertArmed = !alertArmed;
  try {
    localStorage.setItem("as_alert_to", to);
    localStorage.setItem("as_alert_armed", alertArmed ? "1" : "0");
  } catch (e) { /* private browsing — the setting just won't persist */ }
  alertNote(alertArmed
    ? `Armed. <b>${to}</b> will be notified the next time the alarm latches.`
    : "Disarmed. Nothing will be sent.", alertArmed ? "safe" : null);
  paintAlertUI();
});

$("#btnAlertTest").addEventListener("click", () => sendAlert({test: true}));

try {
  $("#alertTo").value = localStorage.getItem("as_alert_to") || "";
  alertArmed = localStorage.getItem("as_alert_armed") === "1";
} catch (e) { /* ignore */ }

/* ===========================================================================
   Wire up the USB control, and be honest when the browser cannot do it
   =========================================================================== */
/* The header button is easy to miss, and it is the one action the whole page
   depends on. The Live tab therefore carries a banner that either offers the
...
function paintConnectBanner() {
  ...
  // A failed attempt is its own state — the generic "press Connect" copy is
  // useless to someone who just pressed Connect and watched it fail.
  if (connectError) {
    banner.hidden = false;
    banner.style.borderColor = "var(--danger)";
    $("#btnUsbBig").hidden = false;
    $("#btnUsbBig").textContent = "Try again";
    $("#connectEyebrow").textContent = "Connection failed";

    if (connectError.kind === "no_port") {
      $("#connectTitle").textContent = "No port was chosen";
      $("#connectWhy").innerHTML =
        "If the picker was <b>empty</b>, the board is not reaching this computer at all. Check " +
        "the USB cable actually carries data — many charging cables do not — and confirm the " +
        "Arduino IDE lists a port under <b>Tools → Port</b>. If it cannot see the board, neither " +
        "can the browser.";
    } else {
      $("#connectTitle").textContent = "Something else is holding that port";
      $("#connectWhy").innerHTML =
        "<code>" + connectError.detail + "</code><br><br>" +
        "Only one program can own a serial port at a time, and something already has this one. " +
        "In order of likelihood:<br>" +
        "<b>1.</b> Quit the <b>Arduino IDE completely</b> — on Windows it keeps the port even " +
        "after you close the Serial Monitor, and closing just the monitor is not enough.<br>" +
        "<b>2.</b> Close <b>any other tab</b> with this dashboard open — a tab you connected " +
        "earlier still owns the port, including one you reloaded away from.<br>" +
        "<b>3.</b> Unplug the Arduino, wait a few seconds, plug it back in, then press " +
        "<b>Try again</b>.";
    }
    return;
  }
  ...
  $("#btnUsbBig").textContent = "Connect detector";
  ...
  $("#connectWhy").innerHTML = SERIAL_SUPPORTED
    ? "Plug the Arduino in, close the Arduino IDE's Serial Monitor, then press " +
      "<b>Connect detector</b> and pick its port. The same button sits in the header."
    : {
        iframe:
          "This page is <b>embedded inside another page</b>, and an embedded frame is never " +
          "allowed to open a USB port. Open the dashboard's own address in its own tab — " +
          "<b>gas-amber-eta.vercel.app</b> — and the button appears.",
        insecure:
          "Opening a USB port needs a <b>secure page</b>. This one is a file on disk or plain " +
          "http. Use the deployed <b>https</b> address, or serve the folder from " +
          "<code>localhost</code>.",
        unsupported:
          "This browser has no Web Serial support. It exists in <b>Chrome, Edge and Opera on a " +
          "desktop or laptop</b> — not Firefox, not Safari, and not any browser on a phone or " +
          "tablet.",
      }[SERIAL_BLOCK];
}
...
if (SERIAL_SUPPORTED) {
  ...
  $("#btnUsbBig").addEventListener("click", connectUsb);
} else {
  const n = $("#usbNote");
  n.hidden = false;
  n.innerHTML = {
